...
class AppController{
    ...
    public function add($formInputs){
        ...
        $object = new $objectName($this->_formInputs);
        //If validation goes well, then add the object to our db
        if ($this->_validation->validate($this->_formInputs, $this->_action) == ValidationController::VALID) {
        ...
        elseif ($this->_validation->validate($this->_formInputs, $this->_action) == ValidationController::EXIST) {
            $this->_actionMessage = $this->_validation->getMessage();    
            $this->_typeMessage = "success";
            $this->_source = $this->_validation->getTarget();
        } 
        //Else throw an error
        elseif ($this->_validation->validate($this->_formInputs, $this->_action) == ValidationController::NOT_VALID) {
            $this->_actionMessage = $this->_validation->getMessage();  
    ...
    public function update($formInputs){
        ...
        //If validation goes well, then add the object to our db
        if ($this->_validation->validate($formInputs, $this->_action) == ValidationController::VALID) {
            $this->_manager->update($object);
            $this->_typeMessage = "success";
        } elseif ($this->_validation->validate($formInputs, $this->_action) == ValidationController::NOT_VALID) {
            $this->_typeMessage = "error";
        }
